...
from core.guardrails import get_max_l2_llm_attempts
from level2.core.prompt_builder_l2 import SYSTEM_PROMPT_L2, build_comparison_prompt, build_l2_correction_prompt
from core.report_generator import save_diff_report_html, save_diff_report_json
from level1.core.schemas import ChangeDirection
from level2.core.schemas_l2 import (
...
from level1.core.validator import build_validation_error_summary, parse_llm_response
from utils.image_utils import encode_image_to_base64, load_and_validate_image, resize_if_needed
    ...
    try:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                if len(validated_findings) >= 5:
                    break
                validation_error_summary = build_validation_error_summary(
                    validation_errors + [f"Only {len(validated_findings)} valid findings produced. Minimum is 5."]
                )
                if attempt < max_attempts and len(validated_findings) < 5:
        ...
        decision_trace.append(f"verdict:{verdict.net_result.value}")
        decision_trace.append(f"a11y_regressions:{verdict.accessibility_regressions_count}")
